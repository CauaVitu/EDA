...
private:
    ...
    // contador de comparações de chaves, para o trabalho.
...
public:
    ...
    // Guardamos todos os pares (chave,valor) num vetor
    vector<std::pair<Key,Value>> iterate(){
     ...
     */
    ChainedHash(size_t tableSize = 19, float load_factor = 1){
    ...
    /// @brief Retorna o numero de comparacoes de chaves realizadas
    ...
    bool contains(const Key& k){
        ...
        for(auto &i:m_table[slot]){
            comparison_counter++;
            //cout << "Comparando: " << i.first << " com " << k << endl; // Debugging output
            if(i.first == k)
                return true;
        }
        return false;
    ...
    // Parece duplicata, mas esse bonitão é const 
    ...
    void rehash(size_t m){
        ...
        if(new_table_size > m_table_size){
            std::vector<std::list <std::pair<Key, Value>>> old_vector = m_table;
            m_table.clear();
            m_number_of_elements = 0;
            m_table_size = new_table_size;
            m_table.resize(m_table_size);
            for (size_t i = 0; i < old_vector.size(); ++i){
                for(auto par: old_vector[i]){
    ...
    Value& operator[](const Key& k){
        ...
        for(auto &par:m_table[slot]){
            comparison_counter++; // Incrementa o contador de comparações
            if(par.first == k){
                return par.second; // se encontrar, retorna o valor associado
            }
        }
        m_table[slot].push_back({k, Value()}); 
        m_number_of_elements++;
        return m_table[slot].back().second; // se não encontrar, insere e retorna o valor padrão
    }
    ...
    const Value& operator[](const Key& k) const{
        return at(k); // chama a funcao const at() que lanca uma excecao se k nao estiver na tabela
    }
